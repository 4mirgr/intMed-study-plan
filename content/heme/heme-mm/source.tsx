...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const c = await window.storage.get("mm-app-card-reviewed");
        if (c?.value) setCardReviewed(JSON.parse(c.value));
      } catch (e) {}
      try {
        const t = await window.storage.get("mm-app-tree-reviewed");
  ...
  const toggleCard = async (itemId) => {
    ...
    try {
      await window.storage.set("mm-app-card-reviewed", JSON.stringify(next));
    } catch (e) {}
  };
  ...
  const toggleTree = async (topicId) => {
    ...
    try {
      await window.storage.set("mm-app-tree-reviewed", JSON.stringify(next));
    } catch (e) {}
  };
